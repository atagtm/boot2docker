...
# download bash-completion too
	wget -O usr/local/share/bash-completion/completions/docker "https://raw.githubusercontent.com/docker/cli/refs/tags/v${DOCKER_VERSION}/contrib/completion/bash/docker"; \
	\
	for binary in \
		containerd \
		ctr \
...
RUN { \
		...
		echo 'HOME_URL="https://github.com/atagtm/boot2docker"'; \
		echo 'SUPPORT_URL="https://forums.freebsd.org"'; \
		echo 'BUG_REPORT_URL="https://github.com/atagtm/boot2docker/issues"'; \
	} > etc/os-release; \
