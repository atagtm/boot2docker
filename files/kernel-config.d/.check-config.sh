...
}
{
    ...
    # kernel 5.1+
    elif is_set CONFIG_LEGACY_VSYSCALL_XONLY; then
        printf -- '- '
        wrap_good "CONFIG_LEGACY_VSYSCALL_XONLY" 'enabled'
        ...
        if [ "$kernelMajor" -gt 5 ] || ([ "$kernelMajor" -eq 5 ] && [ "$kernelMinor" -ge 1 ]); then
    		echo "    $(wrap_color ' "CONFIG_VSYSCALL_XONLY" or use "vsyscall=xonly"' bold black)"
        else
