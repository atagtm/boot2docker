...
# Mount cgroup (if not already mounted)
mountpoint -q /sys/fs/cgroup || \
mount -t cgroup2 cgroup2 /sys/fs/cgroup -o rw,nosuid,nodev,noexec,relatime,nsdelegate,memory_recursiveprot

# Enable all controllers dynamically
controllers=$(cat /sys/fs/cgroup/cgroup.controllers | sed 's/ / +/g')
echo "+$controllers" > /sys/fs/cgroup/cgroup.subtree_control 2>/dev/null || true

# Create the /docker cgroup
mkdir -p /sys/fs/cgroup/docker

# Move (self) PID into docker cgroup
echo $$ > /sys/fs/cgroup/docker/cgroup.procs
